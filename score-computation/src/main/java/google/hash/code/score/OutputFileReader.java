...
public class OutputFileReader {
    ...
    public List<ScoreDrone> parse(Stream<String> fileStream, World world) {
        List<ScoreDrone> drones = new ArrayList<>();
        List<String> collectedLines = fileStream.skip(1).collect(Collectors.toList());
        collectedLines
                .stream()
                .map(s -> s.split(" "))
                .forEach(strings -> {
